...
export const Route = createFileRoute("/_authenticated/facilities")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Facility profiles and current readiness scores." },
...
function FacilitiesPage() {
  const { organization, setFacilityId, can } = useApp();
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      {facilities.length === 0 ? (
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {facilities.map((f) => {
            const score = f.current_readiness_score === null ? null : Number(f.current_readiness_score);
            return (
              <Panel key={f.id} title={f.name} subtitle={f.address ?? undefined}>
                <div className="space-y-5">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="metric text-3xl font-semibold text-foreground">
                      {formatScore(score)}
                    </span>
                    <ReadinessBadge level={score === null ? null : readinessLevelFor(score)} />
                  ...
                  <dl className="grid gap-4 text-sm sm:grid-cols-2">
                    <div>
                      ...
                      <dd className="mt-0.5 text-foreground">{f.primary_products ?? "—"}</dd>
                    </div>
                    <div>
                      <dt className="eyebrow">Primary processes</dt>
                      <dd className="mt-0.5 text-foreground">{f.primary_processes ?? "—"}</dd>
                    </div>
                    <div>
                      <dt className="eyebrow">Machines / Employees / Shifts</dt>
                      <dd className="metric mt-0.5 text-foreground">
                        {f.machine_count ?? "—"} / {f.employee_count ?? "—"} / {f.operating_shifts ?? "—"}
                      </dd>
                    </div>
                    <div>
                      <dt className="eyebrow">Site contact</dt>
                      <dd className="mt-0.5 text-foreground">{f.primary_contact_name ?? "—"}</dd>
                      <dd className="text-xs text-muted-foreground">{f.primary_contact_email}</dd>
                    </div>
                  </dl>
                  ...
                  <div className="flex gap-2 border-t border-border pt-4">
                    <Button size="sm" asChild onClick={() => setFacilityId(f.id)}>
                      <Link to="/dashboard">Open dashboard</Link>
                    </Button>
                    <Button size="sm" variant="outline" asChild onClick={() => setFacilityId(f.id)}>
                    ...
                    {can("manage_organizations") ? (
                      <>
                        ...
                        <Button
                          ...
                          onClick={() =>
                            archive.mutate({ id: f.id, archived: f.status !== "archived" })
                          }
                        >
