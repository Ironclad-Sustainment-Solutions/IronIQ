...
export const Route = createFileRoute("/_authenticated/reports")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Print-ready executive manufacturing readiness report." },
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <PageHeader
        ...
        description={assessment ? `${assessment.name} · ${assessment.assessment_date}` : undefined}
        actions={
          ...
          <Panel title="Executive summary">
            <div className="space-y-4">
              ...
              <p className="text-sm leading-relaxed text-muted-foreground">
                {facility?.name} scored {formatScore(result.overallScore)} out of 100 with a confidence
                score of {formatScore(result.confidenceScore, "%")}, based on {result.answered} scored
                controls across {result.categories.length} weighted categories.
                {result.gated
              ...
              <DefinitionList
                items={[
                  { label: "Confidence score", value: formatScore(result.confidenceScore, "%") },
                  { label: "Completion", value: formatScore(result.completionPct, "%") },
                  { label: "Open findings", value: findings.filter((f) => f.status !== "closed").length },
            ...
            <ul className="divide-y divide-border">
              {ranked.slice(0, 10).map((f) => (
                <li key={f.id} className="flex flex-col gap-1.5 py-3 first:pt-0 last:pb-0">
                  ...
                  <p className="text-xs text-muted-foreground">{f.recommended_action}</p>
                </li>
              ))}
            </ul>
          </Panel>
            ...
            <ul className="divide-y divide-border">
              {projects.map((p) => (
                <li key={p.id} className="flex flex-wrap items-center gap-3 py-3 first:pt-0 last:pb-0">
                  ...
                  <span className="metric text-xs font-semibold text-foreground">{p.percent_complete}%</span>
                </li>
              ))}
            </ul>
          </Panel>
