  ...
  head: () => ({
    meta: [
      { title: "New Assessment — IronIQ" },
      {
        name: "description",
        content:
          "Scope and launch a new manufacturing readiness assessment against a published template version.",
      },
      { property: "og:title", content: "New Assessment — IronIQ" },
      {
        property: "og:description",
        content: "Scope and launch a manufacturing readiness assessment.",
      },
...
function NewAssessment() {
  const { facility, organization, organizations, facilities, profile } =
    useApp();
  ...
  const published = selectableTemplateVersions(
    data?.templates ?? [],
    data?.versions ?? [],
  );
  const templateName = (id: string) => {
    ...
    return template
      ? `${template.template_code ? `${template.template_code} · ` : ""}${template.name}`
      : "Template";
  };
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      if (!form.template_version_id)
        throw new Error("Choose a template version");
    ...
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      <PrerequisiteGate
        requirements={[
          ...
          {
            label:
              "No published assessment template versions yet — publish one before starting an assessment.",
            met: published.length > 0,
            ctaLabel: "Go to templates",
            ctaTo: "/templates",
          },
        ]}
      >
          ...
          >
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={set("assessment_type")}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={set("template_version_id")}
              >
                <SelectTrigger>
