...
import { formatValue, summarizeImprovement, type CapabilityResult } from "@/lib/capability-scoring";
...
export function ReportPanel({
  ...
  const primary = approved.find((f) => f.classification === "primary_constraint") ?? null;

  async function runAi() {
    setBusy(true);
    try {
      const out = (await draftReportNarrative({ data: { context: aiContext } })) as Record<string, string>;
      setNarrative(out);
    } catch (e) {
        ...
        <article className="space-y-6 text-sm">
          <header>
            ...
            <p className="text-muted-foreground">
              {assessment.assessment_date} • Lead assessor: {assessment.lead_assessor ?? "—"}
            </p>
          </header>
          ...
          <Section title="Executive Summary">
            {narrative?.['executive_summary'] ? (
              ...
              <p className="text-muted-foreground">
                Operational capability score {result.overall === null ? "—" : `${result.overall.toFixed(1)} / 5.0`} with{" "}
                {result.severeCount} severe constraint{result.severeCount === 1 ? "" : "s"} identified across{" "}
                {result.domains.filter((d) => d.score !== null).length} assessed capability domains.
              </p>
            )}
          </Section>
          ...
          <Section title="Operational Performance Impact">
            {impacts.length === 0 ? (
              <p className="text-muted-foreground">No performance impacts recorded.</p>
            ) : (
              <ul className="space-y-1">
                {impacts.map((i) => (
                  <li key={i.id}>
                    <strong>{PERF_CATEGORY_LABELS[i.category]}</strong> — {i.current_condition || "—"}
                    {i.metric_name
          ...
          <Section title="Capability Assessment">
            {narrative?.['capability_assessment'] ? <p className="mb-2">{narrative['capability_assessment']}</p> : null}
            <ul className="space-y-1">
              {result.domains.map((d) => (
                <li key={d.domain.id}>
                  <strong>{d.domain.name}</strong> ({d.domain.verb}):{" "}
                  {d.score === null ? "not rated" : `${d.score.toFixed(1)} / 5.0`}
                  {d.severeCount ? ` — ${d.severeCount} severe constraint(s)` : ""}
                </li>
              ))}
            </ul>
          </Section>
          ...
          <Section title="Key Findings">
            {approved.length === 0 ? (
              <p className="text-muted-foreground">No approved findings. Only approved findings appear in the report.</p>
            ) : (
                ...
                {approved.map((f) => (
                  <li key={f.id}>
                    ...
                    <span className="text-muted-foreground">
                      {f.domain_id ? domainById.get(f.domain_id)?.name : "—"} • {f.classification.replaceAll("_", " ")} •{" "}
                      {f.severity} • {f.confidence} confidence • {f.source.replaceAll("_", " ")}
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <Section title="Primary Constraint">
            ...
            {narrative?.['constraint_narrative'] ? <p className="mt-2">{narrative['constraint_narrative']}</p> : null}
          </Section>

          <Section title="Root Capability Gap">
            {gaps.length === 0 ? (
              <p className="text-muted-foreground">No root capability gaps identified.</p>
            ) : (
              <ul className="space-y-2">
                {gaps.map((g) => (
                  <li key={g.id}>
                    ...
                    <br />
                    <span className="text-muted-foreground">
                      Observed: {g.observed_problem}. Immediate cause: {g.immediate_cause ?? "—"}. Consequence:{" "}
          ...
          <Section title="Recommended Restoration Actions & Priority Roadmap">
            {narrative?.['roadmap_narrative'] ? <p className="mb-2">{narrative['roadmap_narrative']}</p> : null}
            {actions.length === 0 ? (
              <p className="text-muted-foreground">No restoration actions defined.</p>
            ) : (
              <ul className="space-y-2">
                {actions.map((a) => {
                  ...
                  return (
                    <li key={a.id}>
                      <strong>[{PRIORITY_LABELS[a.priority]}]</strong> {a.recommended_action}
                      <br />
                      <span className="text-muted-foreground">
                        {a.metric_name ?? "Metric"}: baseline {formatValue(imp.baseline, a.unit)} → target{" "}
                        {formatValue(imp.target, a.unit)} → measured {formatValue(imp.actual, a.unit)} • owner{" "}
                        {a.responsible_party ?? "—"} • {a.status.replaceAll("_", " ")}
                        {val ? ` • sustainment: ${val.result.replaceAll("_", " ")}` : ""}
                      </span>
                    </li>
                  );
                })}
          ...
          <footer className="border-t border-border pt-4 text-xs uppercase tracking-widest text-muted-foreground">
            Ironclad Sustainment Solutions — Restore Capability. Preserve Readiness.®
          </footer>
        </article>
      </Panel>
    </div>
