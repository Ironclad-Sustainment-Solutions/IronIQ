...
export function OverviewPanel({
  ...
  const topAction =
    actions.find((a) => a.priority === "immediate") ??
    actions.find((a) => a.priority === "high") ??
    actions[0] ??
    null;
  const openActions = actions.filter(
    (a) => a.status !== "complete" && a.status !== "sustained",
  );
  ...
  return (
    <div className="grid gap-6">
        ...
        <div className="panel p-5">
          ...
          <div className="mt-3">
            <Meter
              ...
              token={scoreToken(result.overall)}
            />
          </div>
          <p className="mt-2 text-xs text-muted-foreground">
            {result.completionPct}% of ratings complete
          </p>
        </div>
        <StatBox
          label="Severe Constraints"
          value={result.severeCount}
          token={result.severeCount ? "critical" : "success"}
        />
        <StatBox
          label="Open Restoration Actions"
          value={openActions.length}
          token="primary"
        />
        <StatBox
          label="Sustained Capabilities"
          value={sustained}
          token="success"
        />
      </div>
      ...
      {result.severeCount > 0 ? (
        <div className="rounded-md border border-critical/50 bg-critical/10 p-4">
          <p className="flex items-center gap-2 font-display text-sm font-semibold uppercase tracking-widest text-critical">
            <AlertTriangle className="size-4" /> Severe constraints present —
            the overall score does not represent them
          </p>
          <ul className="mt-2 grid gap-1 text-sm text-foreground sm:grid-cols-2">
            {result.severeCriteria.slice(0, 8).map((s, i) => (
              <li key={`${s.criterion.id}-${s.dimension}-${i}`}>
                {s.domain.name} — {s.criterion.name} ({s.dimension}) scored{" "}
                {s.score}
              </li>
            ))}
          </ul>
      ...
      <Panel title="Capability Domains">
        <div className="grid gap-3 md:grid-cols-2">
          {result.domains.map((d) => {
            const domainFindings = findings.filter(
              (f) => f.domain_id === d.domain.id,
            );
            const domainPrimary = domainFindings.find(
              (f) => f.classification === "primary_constraint",
            );
            const domainActions = actions.filter((a) =>
              gaps.some(
                (g) => g.id === a.root_gap_id && g.domain_id === d.domain.id,
              ),
            );
            return (
              <div
                ...
                <dl className="mt-3 grid grid-cols-3 gap-2 text-xs">
                  <div>
                    ...
                    <dd className="font-display text-base text-foreground">
                      {
                        domainFindings.filter((f) => f.severity === "critical")
                          .length
                      }
                    </dd>
                  </div>
                  <div>
                    ...
                    <dd className="font-display text-base text-foreground">
                      {
                        domainActions.filter((a) => a.status !== "sustained")
                          .length
                      }
                    </dd>
                  </div>
                  <div>
                    ...
                    <dd className="text-foreground">
                      {d.weakest?.criterion.name ?? "—"}
                    </dd>
                  </div>
                </dl>
                {domainPrimary ? (
                  <p className="mt-2 text-xs text-critical">
                    Primary constraint: {domainPrimary.title}
                  </p>
                ) : null}
              </div>
            );
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <p className="mt-1 text-foreground">
                  ...
                  {rootGap && !rootGap.validated
                    ? " (suspected — validation required)"
                    : ""}
                </p>
              </div>
              <div>
                ...
                <p className="mt-1 text-muted-foreground">
                  {impacts
                    .map((i) => PERF_CATEGORY_LABELS[i.category])
                    .join(", ") || "—"}
                </p>
              </div>
              {topAction ? (
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 font-display text-xs tracking-wide text-muted-foreground">
                      ...
                      {formatValue(imp.baseline, a.unit)} →{" "}
                      {formatValue(imp.target, a.unit)} →{" "}
      ...
      <CapabilityMap
        ...
        domainName={
          rootGap?.domain_id
            ? (domainById.get(rootGap.domain_id)?.name ?? null)
            : null
        }
        domainQuestion={
          rootGap?.domain_id
...
function CapabilityMap({
  ...
  const imp = action
    ? summarizeImprovement(
        action,
        results.filter((r) => r.action_id === action.id),
      )
    : null;
  const val = action
    ? validations.filter((v) => v.action_id === action.id).at(-1)
    : null;
  ...
  }[] = [
    {
      ...
      facts: [
        ...
        { label: "Where / when visible", value: problem?.q_where_when || "—" },
        {
          ...
          value: problem?.previous_actions || problem?.q_tried || "—",
        },
        {
          label: "Desired outcome",
          value: problem?.desired_outcome || problem?.q_if_resolved || "—",
        },
      ],
    },
    {
      ...
      facts: primary
        ? [
            ...
            { label: "Dimension", value: primary.dimension ?? "—" },
            {
              ...
              value: `${primary.severity} • ${primary.confidence}`,
            },
        ...
        ? [
            {
              ...
              value: rootGap.observed_problem || "—",
            },
            { label: "Immediate cause", value: rootGap.immediate_cause || "—" },
            {
              label: "Contributing factors",
              value: rootGap.contributing_factors || "—",
            },
            {
              label: "Operational consequence",
              value: rootGap.operational_consequence || "—",
            },
          ]
        : [],
    },
        ...
        ? [
            {
              ...
              value: action.responsible_party || "—",
            },
            { label: "Target date", value: action.target_date || "—" },
            {
              label: "Priority / status",
              value: `${action.priority} • ${action.status.replaceAll("_", " ")}`,
            },
            {
              label: "Validation method",
              value: action.validation_method || "—",
            },
          ]
        : [],
    },
  ...
  return (
    <Panel
      ...
      subtitle="Identify → Validate → Restore → Measure → Sustain"
    >
            ...
            <li key={n.label}>
              <div className="rounded-md border border-border">
                <button
                  ...
                  aria-expanded={open}
                >
                  <Tag token={(n.token as "critical") ?? "primary"}>
                    {n.label}
                  </Tag>
                  ...
                  <dl className="grid gap-2 border-t border-border p-3 text-xs sm:grid-cols-2">
                    {n.facts.length === 0 ? (
                      <p className="text-muted-foreground">
                        No supporting detail recorded yet.
                      </p>
                    ) : (
                      n.facts.map((f, idx) => (
                        <div key={`${f.label}-${idx}`}>
