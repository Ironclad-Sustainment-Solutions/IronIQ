            ...
            >
              <div className="flex flex-wrap gap-2">
                ...
                {o.is_pilot_candidate ? (
                  <Badge className="text-[10px] uppercase tracking-wider">Pilot candidate</Badge>
                ) : null}
              </div>
              <AutoField
                label="Title"
              ...
              <AutoField
                ...
                multiline
                disabled={locked}
                onCommit={(capability_gap) => onUpdate(o.id, { capability_gap })}
              />
              <div className="grid gap-3 sm:grid-cols-2">
                <AutoField
                  ...
                  disabled={locked}
                  onCommit={(affected_machines) => onUpdate(o.id, { affected_machines })}
                />
                <AutoField
                  label="Affected parts"
                  value={o.affected_parts}
                  disabled={locked}
                  onCommit={(affected_parts) => onUpdate(o.id, { affected_parts })}
                />
              </div>
              <AutoField
                ...
                multiline
                disabled={locked}
                onCommit={(expected_impact) => onUpdate(o.id, { expected_impact })}
              />
              <AutoField
                ...
                multiline
                disabled={locked}
                onCommit={(recommended_action) => onUpdate(o.id, { recommended_action })}
              />
              <div className="grid gap-3 sm:grid-cols-2">
                <QuickSelect
                  label="Complexity"
              ...
              <QuickSelect
                ...
                disabled={locked}
                columns={3}
                onChange={(workflow_status) => onUpdate(o.id, { workflow_status })}
              />
              <label className="flex min-h-11 items-center gap-2 text-sm text-foreground">
                <input
                  ...
                  disabled={locked}
                  onChange={(e) => onUpdate(o.id, { is_pilot_candidate: e.target.checked })}
                />
                Pilot candidate
              </label>
            </EntryCard>
...
export function PilotTab({
  ...
  onUpdateMetric: (pilotId: string, id: string, values: Partial<PilotMetricRow>) => void;
  ...
  return (
    <div className="space-y-4">
      <Panel
        ...
        actions={
          <Button size="sm" disabled={locked} onClick={() => onAdd({ title: "New pilot" })}>
      ...
      >
        {candidates.length ? (
            ...
            <div className="flex flex-wrap gap-2">
              {candidates.map((c) => (
                <Button
                  ...
                  size="sm"
                  variant="outline"
                  disabled={locked || pilots.some((p) => p.opportunity_id === c.id)}
                  onClick={() =>
                    onAdd({ title: c.title ?? c.opportunity ?? "Pilot", opportunity_id: c.id })
                  }
                >
          ...
          <InlineNote>Flag opportunities as pilot candidates in the implementation backlog.</InlineNote>
        )}
      </Panel>
          ...
          return (
            <Panel
              ...
              actions={
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" onClick={() => setOpenId(open ? null : p.id)}>
                    {open ? "Collapse" : "Expand"}
                  </Button>
                  <Button size="sm" variant="ghost" disabled={locked} onClick={() => onDelete(p.id)}>
                    Delete
                  </Button>
                </div>
              }
            >
              <div className="grid gap-2 sm:grid-cols-4">
                ...
                <StatTile
                  label="Payback"
                  value={roi.paybackWeeks === null ? "—" : `${roi.paybackWeeks} weeks`}
                />
                <StatTile label="ROI" value={roi.roiPercent === null ? "—" : `${roi.roiPercent}%`} />
              </div>
              ...
              {!open ? null : (
                <div className="mt-4 grid gap-4">
                  ...
                  <div>
                    <p className="eyebrow mb-2">Decision aid (1 = low, 5 = high)</p>
                    <div className="grid gap-3 sm:grid-cols-2">
                      {PILOT_SCORES.map((s) => (
                        <ScorePad
                          key={s.key}
                          ...
                          disabled={locked}
                          onChange={(v) => onUpdate(p.id, { [s.key]: v } as Partial<PilotRow>)}
                        />
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <AutoField
                      ...
                      disabled={locked}
                      onCommit={(scope_machine) => onUpdate(p.id, { scope_machine })}
                    />
                    <AutoField
                      label="Scope — fixture"
                      value={p.scope_fixture}
                      disabled={locked}
                      onCommit={(scope_fixture) => onUpdate(p.id, { scope_fixture })}
                    />
                    <AutoField
                      label="Capability gap addressed"
                      value={p.scope_capability_gap}
                      disabled={locked}
                      onCommit={(scope_capability_gap) => onUpdate(p.id, { scope_capability_gap })}
                    />
                  </div>
                  <AutoField
                    ...
                    multiline
                    disabled={locked}
                    onCommit={(scope_outcome) => onUpdate(p.id, { scope_outcome })}
                  />
                  <AutoField
                    ...
                    multiline
                    disabled={locked}
                    onCommit={(scope_exceptions) => onUpdate(p.id, { scope_exceptions })}
                  />
                  ...
                  <div className="grid gap-3">
                    <AutoField
                      ...
                      multiline
                      disabled={locked}
                      onCommit={(current_condition) => onUpdate(p.id, { current_condition })}
                    />
                    <AutoField
                      ...
                      multiline
                      disabled={locked}
                      onCommit={(validated_gap) => onUpdate(p.id, { validated_gap })}
                    />
                    <AutoField
                      ...
                      multiline
                      disabled={locked}
                      onCommit={(proposed_change) => onUpdate(p.id, { proposed_change })}
                    />
                    <div className="grid gap-3 sm:grid-cols-2">
                      <AutoField
                        ...
                        disabled={locked}
                        onCommit={(affected_metric) => onUpdate(p.id, { affected_metric })}
                      />
                      <AutoField
                        label="How success is measured"
                        value={p.validation_method}
                        disabled={locked}
                        onCommit={(validation_method) => onUpdate(p.id, { validation_method })}
                      />
                    </div>
                    <AutoField
                      label="Deliverables"
                      ...
                      multiline
                      disabled={locked}
                      onCommit={(deliverables) => onUpdate(p.id, { deliverables })}
                    />
                    <AutoField
                      label="Exclusions"
                      value={p.exclusions}
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <NumberField
                        ...
                        suffix="USD"
                        disabled={locked}
                        onCommit={(estimated_price) => onUpdate(p.id, { estimated_price })}
                      />
                      <QuickSelect
                        ...
                        disabled={locked}
                        columns={2}
                        onChange={(approval_status) => onUpdate(p.id, { approval_status })}
                      />
                    </div>
                    <QuickSelect
                      label="Implementation status"
                      options={["Not started", "In progress", "Complete", "Validated"]}
                      value={p.implementation_status}
                      disabled={locked}
                      columns={2}
                      onChange={(implementation_status) => onUpdate(p.id, { implementation_status })}
                    />
                    <AutoField
                      ...
                      multiline
                      disabled={locked}
                      onCommit={(implementation_notes) => onUpdate(p.id, { implementation_notes })}
                    />
                  </div>

                  <div>
                    <p className="eyebrow mb-2">Financial assumptions (entered by the assessor)</p>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      <NumberField
                        ...
                        suffix="/hr"
                        disabled={locked}
                        onCommit={(machine_burden_rate) => onUpdate(p.id, { machine_burden_rate })}
                      />
                      <NumberField
                        label="Labor rate"
                        value={p.labor_rate}
                        suffix="/hr"
                        disabled={locked}
                        onCommit={(labor_rate) => onUpdate(p.id, { labor_rate })}
                      />
                      <NumberField
                        ...
                        suffix="/hr"
                        disabled={locked}
                        onCommit={(production_value_hour) => onUpdate(p.id, { production_value_hour })}
                      />
                      <NumberField
                        label="Scrap cost"
                        value={p.scrap_cost}
                        suffix="USD"
                        disabled={locked}
                        onCommit={(scrap_cost) => onUpdate(p.id, { scrap_cost })}
                      />
                      <NumberField
                        label="Overtime cost"
                        ...
                        suffix="USD"
                        disabled={locked}
                        onCommit={(overtime_cost) => onUpdate(p.id, { overtime_cost })}
                      />
                      <NumberField
                        ...
                        suffix="USD"
                        disabled={locked}
                        onCommit={(other_cost_basis) => onUpdate(p.id, { other_cost_basis })}
                      />
                      <NumberField
                        ...
                        suffix="hrs"
                        disabled={locked}
                        onCommit={(hours_recovered_week) => onUpdate(p.id, { hours_recovered_week })}
                      />
                      <NumberField
                        ...
                        suffix="USD"
                        disabled={locked}
                        onCommit={(iss_implementation_cost) => onUpdate(p.id, { iss_implementation_cost })}
                      />
                      <QuickSelect
                        ...
                        disabled={locked}
                        columns={2}
                        onChange={(financial_class) => onUpdate(p.id, { financial_class })}
                      />
                    </div>
                    <div className="mt-3 grid gap-2 sm:grid-cols-4">
                      <StatTile
                        label="Hours recovered / yr"
                        value={roi.annualHours === null ? "—" : String(roi.annualHours)}
                      />
                      <StatTile label="Hourly basis" value={money(roi.hourlyValue)} />
                      <StatTile label="Annual value" value={money(roi.annualValue)} />
                      <StatTile
                        label="Payback"
                        value={roi.paybackWeeks === null ? "—" : `${roi.paybackWeeks} weeks`}
                    ...
                    {roi.missing.length ? (
                      <div className="mt-2">
                        <InlineNote>
                          Not calculated yet — still needed: {roi.missing.join(", ")}.
                        </InlineNote>
                      </div>
                    ) : null}
                  </div>
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          onAddMetric(p.id, {
                            metric_name: p.affected_metric || "Measured outcome",
                    ...
                    {metrics.length === 0 ? (
                      ...
                      <div className="grid gap-3">
                        {metrics.map((m) => {
                          const delta = metricDelta(m.before_value, m.after_value);
                          return (
                            <EntryCard
                              key={m.id}
                              title={m.metric_name}
                            ...
                            >
                              <div className="grid gap-3 sm:grid-cols-3">
                                <AutoField
                                  ...
                                  value={m.metric_name}
                                  disabled={locked}
                                  onCommit={(metric_name) => onUpdateMetric(p.id, m.id, { metric_name })}
                                />
                                <NumberField
                                  label="Before"
                                  value={m.before_value}
                                  disabled={locked}
                                  onCommit={(before_value) => onUpdateMetric(p.id, m.id, { before_value })}
                                />
                                <NumberField
                                  label="After"
                                  value={m.after_value}
                                  disabled={locked}
                                  onCommit={(after_value) => onUpdateMetric(p.id, m.id, { after_value })}
                                />
                              </div>
                              <p
                                className={cn(
                                  "text-sm",
                                  delta.difference === null ? "text-muted-foreground" : "text-foreground",
                                )}
                              >
                                Change:{" "}
