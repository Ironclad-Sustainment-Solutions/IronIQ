...
function version(
  over: Partial<AssessmentTemplateVersion> = {},
): AssessmentTemplateVersion {
  return {
    id: "v1",
    template_id: "t1",
...
describe("category weights", () => {
  it("blocks publishing unless active category weights total exactly 100%", () => {
    const cats = [
      cat({ id: "a", code: "A", weight: 60 }),
      cat({ id: "b", code: "B", weight: 30 }),
    ];
    const questions = [
      q({ id: "q1", category_id: "a" }),
      q({ id: "q2", question_code: "Q2", category_id: "b" }),
    ];
  ...
  it("excludes archived categories from the weight total", () => {
    expect(
      sumCategoryWeights([
        cat({ weight: 100 }),
        cat({ id: "z", weight: 25, archived: true }),
      ]),
    ).toBe(100);
  });

  it("requires at least one category and one active question per category", () => {
    expect(validateForPublish([], []).errors.join(" ")).toMatch(
      /at least one category/i,
    );
    const empty = validateForPublish(
...
describe("create new version", () => {
  it("copies all categories and questions into an incremented draft", () => {
    ...
    const copy = buildVersionCopy(
      { version: version(), categories: cats, questions },
      [
        version({ version: 1 }),
  ...
  it("increments from the highest existing version", () => {
    expect(
      nextVersionNumber([{ version: 1 }, { version: 4 }, { version: 2 }]),
    ).toBe(5);
...
describe("assessments retain their template version", () => {
  it("keeps the original version id when a newer version is published", () => {
    ...
    const resolved = versions.find(
      (v) => v.id === assessment.template_version_id,
    );
...
describe("id and weight validation", () => {
  ...
  it("rejects duplicate category IDs within a version", () => {
    const errors = validateCategoryInput(
      { code: "PC", name: "Process Control", weight: 10 },
      [{ id: "other", code: "pc" }],
    );
    expect(errors.join(" ")).toMatch(/already used/i);
  });
  ...
  it("rejects zero or negative question weights", () => {
    expect(
      validateQuestionInput(
        { question_code: "Q9", question_text: "T", weight: 0 },
        [],
      ).join(" "),
    ).toMatch(/greater than zero/);
    expect(
      validateQuestionInput(
        { question_code: "Q9", question_text: "T", weight: -3 },
        [],
      ).join(" "),
    ).toMatch(/greater than zero/);
  ...
  it("rejects blank required fields", () => {
    expect(
      validateCategoryInput({ code: " ", name: "", weight: 5 }, []),
    ).toHaveLength(2);
  });
});
...
describe("permissions", () => {
  ...
  it("customer admins can only edit templates their own organization owns", () => {
    ...
    const customerOwned = template({
      id: "t2",
      owner_organization_id: "org-1",
      status: "draft",
    });
    expect(
      templatePermissions(["customer_admin"], ironiqOwned, ["org-1"])
        .canEditDraft,
    ).toBe(false);
    expect(
      templatePermissions(["customer_admin"], ironiqOwned, ["org-1"])
    ...
    expect(
      templatePermissions(["customer_admin"], customerOwned, ["org-1"])
        .canEditDraft,
    ).toBe(true);
    expect(
      templatePermissions(["customer_admin"], customerOwned, ["org-2"])
        .canEditDraft,
    ).toBe(false);
    expect(
      templatePermissions(["customer_admin"], ironiqOwned, ["org-1"])
