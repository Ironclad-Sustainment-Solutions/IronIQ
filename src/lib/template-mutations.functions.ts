      ...
      await client.query(
        ...
        [
          ...
          JSON.stringify({
            ...
            template_code: data.values.template_code,
          }),
        ],
      );
      return templateId;
    ...
    withUser(context.userId, async (client) => {
      ...
      await client.query(
        ...
        [
          data.id,
          context.userId,
          data.archived ? "template.archived" : "template.restored",
        ],
      );
    }),
  );
...
export const duplicateTemplate = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) =>
    withUser(context.userId, async (client) => {
      const { rows: templateRows } = await insertReturningId(
        client,
      ...
      const { rows: versionRows } = await insertReturningId(
        client,
      ...
      for (const category of data.categories) {
        const { rows: catRows } = await insertReturningId(
          client,
...
export const publishTemplateVersion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await withUser(context.userId, (client) =>
      client.query("SELECT public.publish_template_version($1)", [
        data.versionId,
      ]),
    );
  });
...
export const deleteDraftVersion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await withUser(context.userId, (client) =>
      client.query(
        "DELETE FROM public.assessment_template_versions WHERE id = $1",
        [data.versionId],
      ),
    );
  });
  ...
  .handler(({ data, context }) =>
    withUser(context.userId, async (client) => {
      if (data.action === "delete") {
        await client.query(
          "DELETE FROM public.assessment_categories WHERE id = $1",
          [data.categoryId],
        );
        return;
      }
      if (data.action === "archive" || data.action === "restore") {
        await client.query(
          "UPDATE public.assessment_categories SET archived = $1 WHERE id = $2",
          [data.action === "archive", data.categoryId],
        );
        return;
      }
      if (data.action === "swap" && data.swapWithId) {
        await client.query(
          "UPDATE public.assessment_categories SET sort_order = $1 WHERE id = $2",
          [data.swapWithSortOrder, data.categoryId],
        );
        await client.query(
          "UPDATE public.assessment_categories SET sort_order = $1 WHERE id = $2",
          [data.ownSortOrder, data.swapWithId],
        );
      }
    }),
  );
...
export const questionRowAction = createServerFn({ method: "POST" })
  ...
  .handler(({ data, context }) =>
    withUser(context.userId, async (client) => {
      if (data.action === "delete") {
        await client.query(
          "DELETE FROM public.assessment_questions WHERE id = $1",
          [data.questionId],
        );
        return;
      }
      if (data.action === "archive" || data.action === "restore") {
        await client.query(
          "UPDATE public.assessment_questions SET archived = $1 WHERE id = $2",
          [data.action === "archive", data.questionId],
        );
        return;
      }
      ...
      if (data.action === "swap" && data.swapWithId) {
        await client.query(
          "UPDATE public.assessment_questions SET sort_order = $1 WHERE id = $2",
          [data.swapWithSortOrder, data.questionId],
        );
        await client.query(
          "UPDATE public.assessment_questions SET sort_order = $1 WHERE id = $2",
          [data.ownSortOrder, data.swapWithId],
