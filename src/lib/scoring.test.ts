...
describe("scoring engine", () => {
  it("validates 0..5 only", () => {
    ...
    [-1, 6, 2.5, "3", null, undefined].forEach((v) =>
  ...
  it("gates Production Ready and Advanced on critical failure", () => {
    expect(applyCriticalGate("Advanced", true).level).toBe(
      "Conditionally Ready",
    );
    expect(applyCriticalGate("Production Ready", true).level).toBe(
      "Conditionally Ready",
    );
    expect(applyCriticalGate("Needs Improvement", true).level).toBe(
      "Needs Improvement",
    );
  ...
  it("keeps the confidence score independent of the readiness score", () => {
    const cats = [cat("A", 100)];
    const qs = [q("a1", "A"), q("a2", "A")];
    const strong = computeAssessmentResult(cats, qs, [
      r("a1", 3, { evidence_type: "system_data" }),
      r("a2", 3, { evidence_type: "system_data" }),
    ]);
    const weak = computeAssessmentResult(cats, qs, [
      r("a1", 3, { evidence_type: "none" }),
      r("a2", 3, { evidence_type: "none" }),
    ]);
...
describe("category weight validation", () => {
  it("accepts weights that total 100 and rejects others", () => {
    expect(
      validateCategoryWeights([{ weight: 60 }, { weight: 40 }]).valid,
    ).toBe(true);
  ...
  it("requires positive question weights", () => {
    ...
    [0, -2, "1", null].forEach((w) =>
      expect(validateQuestionWeight(w)).toBe(false),
    );
  });
});
...
describe("auto-generated findings", () => {
  it("raises critical findings for critical questions scored 0-1 only", () => {
    ...
    expect(autoFindingSeverity({ question: c, response: r("c", 0) })).toBe(
      "critical",
    );
    expect(autoFindingSeverity({ question: c, response: r("c", 1) })).toBe(
      "critical",
    );
    expect(
      autoFindingSeverity({ question: c, response: r("c", 2) }),
    ).toBeNull();
  });

  it("raises high/medium findings for weak non-critical scores", () => {
    const n = q("n", "A");
    expect(autoFindingSeverity({ question: n, response: r("n", 0) })).toBe(
      "high",
    );
    expect(autoFindingSeverity({ question: n, response: r("n", 2) })).toBe(
      "medium",
    );
    expect(
      autoFindingSeverity({ question: n, response: r("n", 3) }),
    ).toBeNull();
  });
  ...
  it("ignores unanswered and N/A questions", () => {
    ...
    expect(
      autoFindingSeverity({
        question: n,
        response: r("n", 0, { not_applicable: true }),
      }),
    ).toBeNull();
  });
